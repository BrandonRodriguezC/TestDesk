...
public class Analizadores {
	...
	public boolean evaluate(boolean compuesta, boolean declaracionAsignacion, String tipoDeVariable, String expresion,
		...
		errores.add((new StringBuilder()).append("LINEA ").append(linea).append(":\nExpresion: ").append(expresion)
				.append("\nCompuesta: ").append(compuesta).append("\nDeclaracion: ").append(declaracionAsignacion)
...
//			System.out.println(rta);
			errores.add((new StringBuilder()).append("INFO L").append(linea).append(": Se espera el tipo de dato <")
					.append(tipoDeVariable).append("> y el tipo generado es <").append(rta).append(">").toString());
			if (rta.equals("X")) {
				errores.add("ERROR L" + (linea + 1) + ": Expresion no operable");
				errores.add("######################################");
				return false;
			} else {
				if (escribir) {
				...
				if (rta.equals(tipoDeVariable) || (tipoDeVariable.equals("R") && rta.equals("E"))) {
					...
					errores.add("ERROR L" + (linea + 1)
							+ ": El resultado de la expresion no\ncoincide con el tipo de la variable");
					errores.add("######################################");
					return false;
				}
			}
			...
			if (posfija.matches(IDENTIFICADOR)) {
				if (tipoDeVariable.equals(encontrarTipoParaIdentificador(posfija.trim(), linea))) {
					errores.add("######################################");
					return true;
				} else {
					errores.add("ERROR L" + (linea + 1)
							+ ": El tipo del identificador no\ncoincide con el tipo de la variable");
					errores.add("######################################");
					return false;
				}
			} else {
				if (!posfija.isEmpty()) {
					...
					if (repetir) {
						...
						if (!expresionlimpia.matches(ENTERO)) {
							errores.add(
									"ERROR L" + (linea + 1) + ": El valor en la expresion repetir\nno es un entero");
							errores.add("######################################");
							return false;
						}
						int valor = 0;
						try {
							...
							errores.add((new StringBuilder())
									.append("ERROR L" + (linea + 1) + ": El valor en la expresion <").append(expresion)
									.append("> no es valido").toString());
							errores.add("######################################");
							return false;
						}

						if (valor < 0) {
							errores.add((new StringBuilder()).append("ERROR: El valor en la expresion <")
									.append(expresion).append("> repetir es menor a 0").toString());
							errores.add("######################################");
							return false;
						} else if (valor == 0) {
							errores.add("ERROR L" + (linea + 1) + ": El valor en la expresion repetir\nes igual a 0");
							errores.add("######################################");
							return false;
						} else {
							return true;
					...
					errores.add("ERROR L" + (linea + 1) + ": La expresion es vacia");
					errores.add("######################################");
					return false;
				}
			}
	...
	public ArrayList<String> analizador(boolean compuesta, boolean declaracionAsignacion, String tipoDeVariable,
			String expresion, int linea) {
...
//DATO RECONOCIDO:
			...
			if (tipoDeDato.equals("ESPACIO")) {
				...
				if (compuesta && declaracionAsignacion) {
					if (estado == 1) {
						...
						errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
								.append(": La <,> (COMA) no se encuentra despues\n de un identificador").toString());
						return null;
					}
				} else {
					errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
							.append(": Separador <,> (COMA) no valido para\neste campo").toString());
					return null;
				}
				...
				if (declaracionAsignacion) {
					errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
							.append(": Parentesis no valido en una declaracion").toString());
					return null;
				}

				if (comparador.group().equals("(")) {
					// errores.add((new StringBuilder()).append("INFO L").append(linea).append(":
					// Parentesis de apertura estado:").append(estado).toString());
					if (estado != 1) {
						if (estado != 4) {
							errores.add((new StringBuilder()).append("INFO L").append(linea + 1)
									.append(": Parentesis de apertura").toString());
							separado.add(comparador.group());
							parentesis++;
							estado = 3;
						} else {
							errores.add((new StringBuilder()).append("ERROR L").append(linea + 1).append(
									": No hay operador entre el parentesis de cierre y el parentesis de apertura")
									.toString());
							return null;
						}
					} else {
						errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
								.append(": Parentesis de apertura posterior a tipo de dato").toString());
						return null;
					}
					...
					if (parentesis > 0) {
						if (estado != 0) {
							if (estado != 2) {
								if (estado != 3) {
									errores.add((new StringBuilder()).append("INFO L").append(linea + 1)
											.append(": Parentesis de cierre").toString());
									parentesis--;
									estado = 4;
									separado.add(comparador.group());
								} else {
									errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
											.append(": Expresion vacia entre parentesis").toString());
									return null;
								}
							} else {
								errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
										.append(": No hay identificador antes del parentesis de cierre").toString());
								return null;
							}
						} else {
							errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
									.append(": Parentesis de cierre al inicio de expresion").toString());
							return null;
						}
					} else {
						errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
								.append(": Parentesis de cierre sin parentesis de apertura presente").toString());
						return null;
					}
				}
					...
					if (comparador.group().length() <= 15) {
						if (!comparador.group().matches(PALABRAS_PROHIBIDAS)) {
							if (!estaEnTabla) {
								...
								errores.add((new StringBuilder()).append("INFO L").append(linea + 1)
										.append(": Se añadio el identificador en la tabla\nde simbolos <").append(llave)
										.append(":").append(tipoDeVariable).append(">").toString());
								estado = 1;
								separado.add(comparador.group());

							} else {
								errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
										.append(": El identificador ya fue declarado ").append(llave).toString());
								return null;
							}
						} else {
							errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
									.append(": El nombre de este identificador <").append(comparador.group())
									.append(">\nes una palabra reservada").toString());
							return null;
						}
					} else {
						errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
								.append(": El identificador es mayor a 15 caracteres").toString());
						return null;
					}

				} else if (estaEnTabla) {
					errores.add((new StringBuilder()).append("INFO L").append(linea + 1)
							.append(": Se encontro el identificador <").append(comparador.group())
							.append("> en la tabla").toString());
					estado = 1;
					separado.add(comparador.group());

				} else {
					errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
							.append(": No se encontró el identificador <").append(comparador.group())
							.append("> en la tabla").toString());
					return null;
				}
			} else if (tipoDeDato.equals("OPERADOR") && comparador.group().equals("!")){
				
				if (estado == 3 || (contador == 1 && estado == 0)) {
					errores.add((new StringBuilder()).append("INFO L").append(linea + 1).append(": Operador unario")
							.toString());
					estado = 2;
					contador--;
					separado.add(comparador.group());
					...
					errores.add((new StringBuilder()).append("ERROR L").append(linea + 1).append(
							": Operador unario no precede a un parentesis de apertura,\n no es inicial en la expresion y tampoco es precedido por un operador")
							.toString());
					return null;
				}
				...
				if (declaracionAsignacion) {
					errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
							.append(": Operador no valido en declaración").toString());
					return null;
				}
				...
				if (comparador.group().equals("-")) {
					if (estado == 3 || (contador == 1 && estado == 0)) {
						...
						errores.add((new StringBuilder()).append("ERROR L").append(linea + 1).append(
								": Operador unario no precede a un parentesis de apertura,\n no es inicial en la expresion y tampoco precede un identificador o valor")
								.toString());
						return null;
					}
				}else if (contador > 1) {
					if (estado != 2) {
						if (estado != 3) {
							errores.add((new StringBuilder()).append("INFO L").append(linea + 1)
									.append(": Operador no inicial").toString());
							estado = 2;
							separado.add(comparador.group());
						} else {
							errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
									.append(": El operador precede un parentesis de apertura").toString());
							return null;
						}

					} else {
						errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
								.append(": El operador precede un operador").toString());
						return null;
					}

				} else {
					errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
							.append(": El operador es inicial en la expresion").toString());
					return null;
				}
				...
				if (!declaracionAsignacion) {
					if (estado != 1) {
						if (estado != 4) {
							...
							errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
									.append(": Dato simple precede un parentesis de cierre").toString());
							return null;
						}
					} else {
						errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
								.append(": Dato simple precede un identificador").toString());
						return null;
					}
				} else {
					errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
							.append(": Dato simple no valido en una declaración").toString());
					return null;
				}

			} else {
				errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
						.append(": Error no identificado (multiple) ").append(comparador.group()).toString());
				return null;
			}
			...
			if (contador == 0 || contador == 1) {
				if (comparador.start() == 0) {
					...
					errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
							.append(": Simbolo no identificado <")
							.append(expresion.substring(ultimoCaracterIdentificado, comparador.start())).append(">")
							.toString());
					return null;
				}
			}
			...
			if (!compuesta && contador > 1) {
				errores.add((new StringBuilder()).append("ERROR L").append(linea + 1)
						.append(": Expresion compuesta en campo simple").toString());
				return null;
			}
		}

		if (estado == 2) {
			errores.add((new StringBuilder()).append("ERROR L").append(linea + 1).append(": Termina en operador")
					.toString());
			return null;
		}
		...
		if (estado == -1) {
			errores.add(
					(new StringBuilder()).append("ERROR L").append(linea + 1).append(": Termina en coma").toString());
			return null;
		}

		if (parentesis != 0) {
			errores.add((new StringBuilder()).append("ERROR L").append(linea + 1).append(": Error en uso de parentesis")
					.toString());
			return null;
		}
	...
	public String encontrarTipoParaIdentificador(String identificador, int linea) {
		if (tablaDeSimbolos.containsKey(tablaDeSimbolos.hash(identificador))) {
			...
			errores.add("ERROR " + (linea + 1) + ": No se encuentra el identificador\n \"" + identificador + "\"");
			return "ERROR";
		}
	}
