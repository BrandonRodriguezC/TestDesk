...
public class CodeArea extends StyledTextArea {
	...
	private int ciclos = 0, rangoDeEscrituraInicio1 = 0, rangoDeEscrituraFin1 = 0, rangoDeEscrituraInicio2 = 0,
...
//	ArrayList<String> estructura;
	...
	private final static Pattern PATTERN = Pattern.compile("(?<KEYWORD>" + String.join("|", keyword) + ")"
			+ "|(?<PARENTESIS>" + BRACE_PATTERN + ")" + "|(?<TIPODEDATO>" + String.join("|", tipoDeDato) + ")"
			+ "|(?<LEER>" + LEER + ")" + "|(?<ASIGNACION>" + ASIGNACION + ")" + "|(?<COMENTARIO>" + COMENTARIO + ")"
			+ "|(?<CIERRE>" + CIERRE + ")" + "|(?<CIERREBLOQUE>" + CIERREBLOQUE + ")" + "|(?<APERTURABLOQUE>"
			+ APERTURABLOQUE + ")" + "|(?<ESCRIBIR>" + ESCRIBIR + ")");
...
//		ctrl.presentarEstructura(estructura);
	...
	public void estilizarLinea(String linea, int numeroLinea) {
		if (!linea.isEmpty()) {
			...
			tipo_expresion_int = linea.matches("(\t+)?(\s+)?.*(\s+)?\\=.*\\;")
					&& linea.matches("((?!(entero|logico|real|texto|escribir)).)*") ? 0 : // cambiar a [^ ]+ evitar
																							// errores de remocion
							linea.matches("(\t+)?(\s+)?(entero|logico|real|texto)(\s+)?.*(\s+)?\\=.*\\;") ? 1 : // cambiar
																												// a [^
																												// ]+
																												// evitar
																												// errores
																												// de
																												// remocion
									linea.matches("(\t+)?(\s+)?(entero|logico|real|texto)(\s+)?.*(\s+)?\\;") ? 2 : // cambiar
																													// a
																													// [^
																													// ]+
																													// evitar
																													...
																													// remocion
											linea.matches(
													"(\t+)?(si |mientras que |repetir )(\\().*(\\))(\s+)?(veces)?")
													|| linea.matches("(\t+)?(sino)") ? 3
															: linea.matches("(\t+)?(escribir|leer)(\\().*(\\))\\;") ? 4
																	: linea.matches("(\t+)?[\\} | \\{]") ? 5 : -1;
			/**
			 * DICCIONARIO TIPO DE EXPRESION: - ASIGNACION = 0 - DECLARACION-ASIGNACION = 1
			 * - DECLARACION = 2 - BLOQUE = 3 - METODO = 4 - LLAVE = 5 - NO_RECONOCIDO = -1
			 */
				...
				while (comparador.find()) {
													...
													: comparador.group("ASIGNACION") != null ? "ASIGNACION"
															: comparador.group("COMENTARIO") != null ? "COMENTARIO"
																	: comparador.group("CIERRE") != null ? "CIERRE"
																			: comparador.group("CIERREBLOQUE") != null
																					? "CIERRE_BLOQUE"
																					: comparador.group(
																							"APERTURABLOQUE") != null
																									? "APERTURA_BLOQUE"
					...
					if (tipo.equals("KEYWORD")) {
						if (inicioExpresion == -1) {
							estilos.add(new StyleRange("keyword", offsetLinea + comparador.start(),
									comparador.group().length(), null, null));
//							DEBUG
...
//										System.out.println(tipo_expresion_str+" "+tipo_expresion_int);
						...
						if (inicioExpresion == -1) {
							estilos.add(new StyleRange("keyword", offsetLinea + comparador.start(),
									comparador.group().length() - 1, null, null));
//							DEBUG
...
//					estructura.add(numeroLinea+" \t|\t|__\tExpresion 1: "+ expresion1);
					...
					if (tipo_expresion_int == 0) {
						tipo_variable_esperado = ctrl.encontrarTipoParaIdentificador(expresion1.trim(),numeroLinea);
					}
					boolean resultado = analisisExpresion(tipo_expresion_int, 1, expresion1, tipo_variable_esperado,
							numeroLinea);
...
//					System.out.println(expresion1+ " expresion 2: "+numeroLinea );
					boolean resultado = analisisExpresion(tipo_expresion_int, 2, expresion2, tipo_variable_esperado,
							numeroLinea);
					estilizarExpresiones(estilos, resultado, numeroLinea);
				}
//				estructura.add(numeroLinea+" \t|\t\t");
	...
	public boolean analisisExpresion(int tipo_expresion, int campo, String expresion, String tipo_variable_esperado,
			int numero_linea) {
...
//				System.out.println("\t\t\tEstilo " + estilo + " " + numeroLineaExpresion + ": " + (rangoDeEscrituraInicio1 + 1) + "-" + (rangoDeEscrituraFin1 - 1));
				ar.add(new StyleRange(estilo, rangoDeEscrituraInicio1 + 1,
						rangoDeEscrituraFin1 - rangoDeEscrituraInicio1 - 1, null, null));
				camposEstilados++;
				return;
			}
...
//				System.out.println("\t\t\tEstilo " + estilo + " " + numeroLineaExpresion + ": " + (rangoDeEscrituraInicio2 + 1) + "-" + (rangoDeEscrituraFin2 - 1));
				ar.add(new StyleRange(estilo, rangoDeEscrituraInicio2 + 1,
						rangoDeEscrituraFin2 - rangoDeEscrituraInicio2 - 1, null, null));
				camposEstilados = 0;
				return;
			}
		}
...
//						System.out.println("Anterior: "+lineaAnterior+" "+ultimaLineaEditada);
					...
					if (ultimaLineaEditada.matches("((?!(entero|logico|real|texto)).)*")) {
						...
						// SI CONTIENE IDENTIFICADOR , REMOVER ESPACIOS ANTES DEL IDENTIFICADOR Y EN EL
						// IGUAL
						...
						if (!sinEspacios.matches("^(\t+)?\s+\\=.*")) {
							...
							Arrays.fill(tab, '\t');
							sinEspacios = (new StringBuilder())
									.append(sinEspacios.replaceAll("^(\t+)?\s+", new String(tab))).toString();
						...
						// SI CONTIENE IDENTIFICADOR Y PUNTO Y COMA, REMOVER ESPACIOS ENTRE TIPO DE DATO
						// Y PUNTO Y COMA
						String tipo = "entero";
						...
						if (ultimaLineaEditada.matches(
								"(\t+)?(entero|real|logico|texto)\s+([a-z]+([0-9]+)?)\s*(?:\\,\s*[a-z]+([0-9]+)?)*\s+\\;")) {
							sinEspacios = sinEspacios.replaceAll("(entero|real|logico|texto)\s+", tipo + " ")
									.replaceAll("\s+\\;", ";");
						}

						// SI CONTIENE IDENTIFICADOR E IGUAL, REMOVER ESPACIOS ENTRE TIPO DE DATO E
						// IGUAL
...
//							System.out.println("Si se reemplazó");
				...
				// SI CONTIENE TIPO DE DATO ES DECLARACION
					...
					// SI CONTIENE IDENTIFICADOR Y PUNTO Y COMA, AÑADIR ESPACIOS ENTRE TIPO DE DATO
					// Y PUNTO Y COMA
					String tipo = "entero";
...
//			estructura.add("ASIGNACION");
			...
			añadirRangoEscritura(getOffsetAtLine(lineaActual) + sb.indexOf("="),
					getOffsetAtLine(lineaActual) + sb.indexOf(";"), 2);
...
//			estructura.add("DECLARACION Y ASIGNACION");
			...
			añadirRangoEscritura(getOffsetAtLine(lineaActual) + sb.indexOf(tipo) + tipo.length() + 1,
...
//			estructura.add("DECLARACION");
			...
			añadirRangoEscritura(getOffsetAtLine(lineaActual) + sb.indexOf(tipo) + tipo.length(),
					getOffsetAtLine(lineaActual) + sb.indexOf(";"), 1);
